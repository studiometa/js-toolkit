...
class Tooltip extends Base {
  ...
  async setPosition(position) {
    ...
    return Promise.all(Object.entries(this.$options.styles).map(([refName, value = {}]) => {
      if (!value[position]) {
        return Promise.resolve(this);
      }

      return transition(this.$refs[refName], {
  ...
  async open() {
    ...
    const {
      x,
      y: position
    } = findBestPosition(this.$refs.content, this.$options.offset);
  ...
  async close() {
    ...
    return Promise.all(Object.entries(this.$options.styles).map(([refName, {
      ...
      active,
      to: closed
    }, 'keep'))).then(async function () {
      await _this.setPosition('top');

      _this.$emit('close');

      _this.$refs.container.setAttribute('aria-hidden', 'true');
