...
| `js-toolkit/no-deep-utils-import`                 | Disallows imports reaching into the package layout under `@studiometa/js-toolkit/utils/`. Use the per-symbol subpath `@studiometa/js-toolkit/utils/<name>`, or the `utils` barrel for several. | error       |         |
| `js-toolkit/no-internal-barrel-import`            | Disallows relative imports of an internal barrel (`./utils/index.js`). Import the module declaring the symbol instead. For libraries published as unbundled ES modules.                        | off         |         |
