...
- **A declared option outranks a derived spelling.** `data-option-no-sort` is `noSort`'s own attribute wherever `noSort` is declared — `@studiometa/ui`'s `StoreLocator` does declare it — so the derived negation stands down and says so. Resolving it silently either way would make one attribute mean two things, and the whole point of a live attribute is that its meaning is legible from the markup.
