...
12. **`selectorFor(name)` or `getInstances(name, root = document)`** (gap 20). Ten lines. `Action` is unwritable in v4 without resolving components by name across the whole page, and the DOM sweep that does it is fine — what is not fine is every ui component that resolves by name hard-coding `[data-component~="…"]` because core keeps its own selector private.
13. **A way to observe an attribute the framework did not name** (gap 21). `data-on:<event>` is `Action`'s primary documented form and is invisible to the `attributeFilter`, so a morph or a `data-bind:` template leaves a stale binding behind with no warning. Exporting `registerDOMOptionAttributes` is the cheap version.
