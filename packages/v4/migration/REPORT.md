...
| `data-ref="dots[]"` → `data-ref="dots"` **in the markup**         | v4 declares an array ref as `dots[]` in `config.refs` and selects `[data-ref="dots"]`; v3 kept the suffix in both places. **This is a template change, not a JavaScript one, and it is the widest mechanical edit found so far: 44 attributes across 19 files in ui.** It fails silently — the refs resolve to an empty array and the component does nothing.                                                                                                                                                                                                                                                                            |
...
11. **An array ref lost its `[]` in the markup.** `config.refs: ['dots[]']` now selects `[data-ref="dots"]`; v3 selected `[data-ref="dots[]"]`. **44 attributes across 19 template files in ui**, it is a template change rather than a code one, and it fails silently: the ref resolves to `[]` and the component does nothing. **Ask:** accept both spellings during the migration, or a codemod for the templates plus a dev warning when an element declares `data-ref="x[]"`.
...
9. **A codemod for `data-ref="x[]"`** (gap 11), and `{ axis }` on `useDrag` (gap 12). The first is 44 silent breakages in ui's templates; the second is one CSS declaration every draggable component would otherwise get subtly wrong.
