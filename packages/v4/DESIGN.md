...
Core keeps what the rest of the ported components need from the maths: the easing functions on `utils/easings.ts` (ported 2026-08-16), `spring()` and `smoothTo()`. `migration/utils/keyframes.ts` and the `cubicBezier` beside it stay in `migration/` until the `ScrollAnimation` family goes.
