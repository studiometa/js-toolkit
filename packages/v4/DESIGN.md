...
- **A class resolves to `config.name`** — the same string the string form resolves to, landing on the same delegated entry. It is not a parallel path, it is the same one with the name read off the class instead of typed out. What it adds is the type: the class _is_ the type, so `target` is the component and `payload` is read from its `$emits` with nothing to annotate. `@on(window, 'click')` types the event from `WindowEventMap` and falls back to `Event` — never `any` — for a custom name.
