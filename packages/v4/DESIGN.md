...
- **A `target` resolver is typed against the host as declared, and that is a limit worth knowing.** A mixin is applied while the extends clause of its class is still being evaluated, so `withDrag(Base, …)` types its resolver against `Base`: a component reaching further names the shape it needs (`(instance as Base & { readonly target: HTMLElement }).target`), which is an assertion rather than a check. v3 writes the same line as an `@ts-expect-error`.
  **So core checks what the assertion cannot.** A resolver the caller supplied which comes back with `undefined` or `null` reports `service.missing-target` and starts no subscription — because a renamed ref arrives here as nothing, and every service with a default target (`useResize()` defaults to the document element) would otherwise observe the wrong thing and look like it worked. A service whose **own** default target is nothing — `withRaf` — is untouched: that is the contract, and only a caller's resolver can be wrong about it.

- **A mixin names one hook per class, and that is its limit.** It binds one subscription, under the one name the service owns, for each mount cycle. A component whose subscriptions are **one per markup declaration** — `Track`'s `data-track:view`, one per attribute with its own modifiers and its own threshold, known only when the element is read — has no method to name and no fixed count, so it calls `subscribe()` itself and returns the release from `mounted()`. That is the intended path rather than a workaround, and reaching for it deliberately is cheaper than discovering that a hook cannot be dynamic.

  **`on<Event>` has the same shape**: a handler name belongs to the class, while a set of events can be data. Both sugars are keyed on a name a class declares, and both leave the same escape open — bind it yourself, own the cleanup. Neither is about a build step: `withRaf(Base)` is an ordinary call and the name is fixed by how the class is written, which is as true without a compiler as with one.
