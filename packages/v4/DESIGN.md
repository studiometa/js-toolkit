...
**The keyframes interpolator is not promoted (2026-08-16).** It was on the list above — `compile(keyframes, { easing }) => (progress, size) => styles`, ~150 lines including the `cubicBezier` which replaces `@motionone/easing`. `ui-motion` ships that tooling instead, which is where its one consumer (`AbstractScrollAnimation`) and the rest of the engine already live. Core keeps only what the ported components need from the maths: the easing functions on `utils/easings.ts`, `spring()` and `smoothTo()`. `migration/utils/keyframes.ts` and `migration/utils/easings.ts` stay where they are, as the measurement they were.
