...
- **What is deliberately not ported.** v3's `spring()` has the same defect and needs a different fix, so it stays out until something needs it: `dt` appears nowhere in it, making it a pure step recurrence whose shape is preserved but whose rate doubles with the display (measured: settles in 56 real frames at 60 Hz, 28 at 120 Hz, identical `104.24` overshoot at both). Its fix is fixed-timestep substepping or the analytic solution, not this one — and it is unstable besides, with no guard: `stiffness: 1.9` overshoots to `190` and `stiffness: 4` diverges to `-1.6e15`. v3's `smoothTo()` is worse and would need rewriting rather than porting: `update()` calls `tick()` synchronously and `tick()` re-schedules itself through `requestAnimationFrame` with nothing cancelling or de-duplicating, so **every** call while the value is settling starts another self-perpetuating chain — five updates in one frame measured five chains, five notifications per frame, and five more frames queued. It also owns a `requestAnimationFrame` loop, which this section forbids.
