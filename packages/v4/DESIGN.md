...
- **The string form is the one a lazy child wants.** A child declared as `Child: () => import('./Child.js')` exists to keep its chunk out of the parent's; `@on(Child, 'open')` would import the very chunk the thunk defers, so `@on('Child', 'open')` is what to write. The thunk is not itself a target — a function with no `config`, refused by the overloads and at runtime.
- **A name is a child _or_ a ref**, resolved children-first, so the string form types its handler as either `DelegatedEvent` or `RefEvent`. A list ref is named the way every handler form names it — `@on('dots', 'click')` for a `dots[]` declaration and a `data-ref="dots[]"` attribute. The class form has no such doubt: a class can only be a child.
