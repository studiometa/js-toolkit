...
describe('The Options class', () => {
  ...
  it('should get and set regexp options', () => {
    const instance = componentWithOptions('<div data-option-foo="/bar/"></div>', {
      foo: RegExp,
    });

    expect(instance.$options.foo).toEqual(new RegExp('bar'));
    instance.$options.foo = new RegExp('baz');
    expect(instance.$el.getAttribute('data-option-foo')).toBe('baz');
    expect(instance.$options.foo).toEqual(new RegExp('baz'));
  });
