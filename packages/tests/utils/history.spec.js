import { describe, beforeEach, it, expect, spy, sinon } from 'vitest';
...
describe('The history `push` method', () => {
  ...
  it('should work', () => {
    ...
    expect(window.location.href).toBe('http://localhost:3000/bar?query=baz&baz=false#foo');
  });
  ...
  it('should remove search params when their value is null, undefined or an empty string', () => {
    ...
    expect(window.location.href).toBe(
      'http://localhost:3000/?query=foo&nullish=foo&notDefined=foo&false=true'
    );
    push({
      search: { query: '', notPresent: '', nullish: null, notDefined: undefined, false: false },
    });
    expect(window.location.href).toBe('http://localhost:3000/?false=false');
  });

  it('should remove the hash when none given', () => {
    window.history.replaceState({}, '', '/#foo');
    expect(window.location.href).toBe('http://localhost:3000/#foo');
    push({ hash: '' });
    expect(window.location.href).toBe('http://localhost:3000/');
  });
  ...
  it('should convert arrays and objects to valid PHP $_GET params', () => {
    ...
    expect(decodeURI(window.location.href)).toBe(
      'http://localhost:3000/?array[0]=1&array[1]=2&array[2][obj]=true&object[foo]=foo&object[bar][baz]=bar'
    );
  });
  ...
  it('should pass the data and title to the history API', () => {
    const pushMock = spy(window.history, 'pushState');
  ...
  it('should accept URLSearchParams instance as search', () => {
    ...
    search.set('foo', 'bar');
    push({ search });
    expect(window.location.href).toBe('http://localhost:3000/?foo=bar');
  });
});
...
describe('The history `replace` method', () => {
  ...
  it('should work', () => {
    ...
    expect(window.location.href).toBe('http://localhost:3000/bar?query=baz#foo');
  });
});
